			...
			<?php if(isset($_SESSION['username']) && ($_SESSION['user_status']==1))
				...
				<ul>
				    ...
				    <?php $database= new MySqlDatabase();
                                    ...
                                    $rez=$database->query("SELECT * FROM car_makes");

                                    while ($result=$database->fetch_array($rez))
                                        { 
                                        
                                       // $models[$bla['car_makes_id']].="\"" .$bla["car_model_name"]."\", ";
                                       // $models_id[$bla['car_makes_id']].="\"" .$bla['car_model_id']."\", ";
                                       
                                       echo "<li><a href='simple.php?sr_mark=".$result['car_makes_id']."&sr_model=&year_from=&year_to=&page=1'><h3>".$result['car_name']."</h3></a></li>";
                                        }
