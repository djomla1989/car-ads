...
<div class="shell">
	...
	<div id="content">
		...
		<!-- Main -->
                <div id="main">
                     <div class="box">
                         <h2>Dodavanje modela vozila</h2>
                         <form style="padding-left: 5px;" method="post" action="add_model.php">
                            Proizvodjaci <br/>
                       <select name='car_model' onChange='update_models(this.selectedIndex);'>
                           <option value="%">Sve</option>
                                        <?php
                                        $database=new MySqlDatabase();
                                        $rez=$database->query("SELECT * FROM car_makes ORDER BY car_name");

                                            while ($bla=$database->fetch_array($rez))
                                        {
                                        echo "<option value='".$bla['car_makes_id']."'>".$bla['car_name']."</option>";
                                        }
                                        ?>

                      </select> 
                      <h3>Ime modela:</h3><br/> <input type="text" name="nova_marka"/><br/><br/>
                          <input style="width: 50px; margin-left: 35px;" type="submit" value="Dodaj" name="Dodaj_model"/>
                    </form>
                     </div>
                  	<?php

                        if(isset($_POST['Dodaj_model']))
                            {
                            $nesto=CarModels::add_car_model($_POST['car_model'],$_POST['nova_marka']);
                           if($nesto)
